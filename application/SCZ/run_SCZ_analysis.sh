...
do
	...
	CONFIG_FILE="/project2/mstephens/wcrouse/UKB_analysis_allweights_scz/ctwas_config.R"
	OUTNAME_E=$TRAIT"_"$WEIGHT"_expr"
	OUTNAME=$TRAIT"_"$WEIGHT"_ctwas"
	OUTDIR="/project2/mstephens/wcrouse/UKB_analysis_allweights_scz/"$TRAIT"/"$WEIGHT"_nolnc"
	...
	if [ ! -f $FINAL_FILE ]
	then 
		echo sbatch --export=TRAIT_FILE=$TRAIT_FILE,LD_DIR=$LD_DIR,WEIGHT_FILE=$WEIGHT_FILE,CONFIG_FILE=$CONFIG_FILE,OUTNAME_E=$OUTNAME_E,OUTNAME=$OUTNAME,OUTDIR=$OUTDIR --job-name=$job_name --output=$job_out --error=$job_err run_SCZ.sbatch
	fi
done
