---
title: "Inflammatory bowel disease - all weights (no lncRNA)"
author: "wesleycrouse"
date: "2022-06-09"
...
pdf(file = "output/Figure_S17a.pdf", width = 3.5, height = 2.5)
...
pdf(file = "output/Figure_6a.pdf", width = 4, height = 3)
...
pdf(file = "output/Figure_S15a", width = 4, height = 3)
...
pdf(file = "output/Figure_6d.pdf", width = 2.75, height = 3.5)
...
pdf(file = "output/Figure_S14a.pdf", width = 7, height = 5)
...
write.csv(GO_enrichment_out, file="output/Table_S11.csv", row.names=F)
...
write.csv(GO_enrichment_silver_standard_out, file="output/Table_S12.csv", row.names=F)
...
FUMA <- FUMA[,-c("link")]
write.csv(FUMA, file="output/Table_S13.csv", row.names=F)
...
write.csv(results_summary_out, file="output/Table_S9.csv", row.names=F)
...
write.csv(results_summary_compact, file=paste0("output/Table_1.csv"), row.names=F)
...
pdf(file = "output/Figure_6c.pdf", width = 2.75, height = 3)
par(mar=c(4.6, 3.6, 1.1, 0.6))
...
pdf(file = "output/Figure_6e.pdf", width = 4.5, height = 3)
...
write.csv(webgestalt_full, file="output/Table_S10.csv", row.names=F)
```

## Locus plots for paper
...
locus_plot <- function(genename=NULL, tissue=NULL, xlim=NULL, return_table=F, label_panel="TWAS", label_genes=NULL, label_pos=NULL, plot_eqtl=NULL, draw_gene_track=T, legend_side="right", legend_panel="cTWAS", twas_ymax=NULL){
...
locus_plot_gene_track <- function(a, label_pos=NULL, filter_noncoding=T){
  chr <- unique(a$chrom)
...
pdf(file = "output/Figure_6f.pdf", width = 5, height = 3.5)

a <- locus_plot(genename="UBE2W", 
...
pdf(file = "output/Figure_6f_genetrack.pdf", width = 3.86, height = 0.5)
biomTrack <- locus_plot_gene_track(a)
dev.off()

pdf(file = "output/Figure_6f_genetrack_v2.pdf", width = 3.86, height = 0.5)
biomTrack <- locus_plot_gene_track(a, label_pos="right")
dev.off()


#check if UBE2W is significant by TWAS FDR
analysis_id <- paste(trait_id, "Colon_Transverse", sep="_")
results_dir <- results_dirs[grep("Colon_Transverse", results_dirs)]
load(paste0(results_dir, "/", analysis_id, "_expr_z_gene.Rd"))

z_gene$p <- 2*pnorm(abs(z_gene$z), lower.tail=F, log.p=F)
z_gene$p_adjust <- p.adjust(z_gene$p, method="BH")

#UBE2W p-value
z_gene$p[grep(results_summary$ensembl_gene_id[results_summary$genename=="UBE2W"], z_gene$id)]

#UBE2W significant by FDR?
z_gene$p_adjust[grep(results_summary$ensembl_gene_id[results_summary$genename=="UBE2W"], z_gene$id)] < 0.05
...
pdf(file = "output/Figure_S18b.pdf", width = 5, height = 3.5)

a <- locus_plot(genename="LSP1", 
...
pdf(file = "output/Figure_S18b_genetrack.pdf", width = 3.86, height = 0.5)
biomTrack <- locus_plot_gene_track(a, label_pos="above")
dev.off()
...
pdf(file = "output/Figure_S18a.pdf", width = 5, height = 3.5)

a <- locus_plot(genename="TYMP", 
...
pdf(file = "output/Figure_S18a_genetrack.pdf", width = 3.86, height = 0.7)
biomTrack <- locus_plot_gene_track(a, label_pos="right", filter_noncoding=F)
dev.off()
...
pdf(file = "output/Figure_S18c.pdf", width = 5, height = 3.5)

a <- locus_plot(genename="CCR5", 
                          tissue="Whole_Blood", 
                          return_table=T, 
                          legend_side="left", 
                          xlim=c(46.1, 46.5),
...
pdf(file = "output/Figure_S18c_genetrack.pdf", width = 3.86, height = 0.5)
biomTrack <- locus_plot_gene_track(a, label_pos="above", filter_noncoding=F)
dev.off()
